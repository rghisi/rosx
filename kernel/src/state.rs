...
impl ExecutionState {
    ...
    pub(crate) fn switch_to_task(&mut self, task_handle: TaskHandle) -> TaskHandle {
        let task_stack_pointer = TASK_MANAGER.lock().borrow().get_task_stack_pointer(task_handle);
        self.current_task = Some(task_handle);
        let scheduler_stack_pointer_pointer = TASK_MANAGER.lock().borrow_mut().get_task_stack_pointer_ref(self.main_thread);
    ...
    pub(crate) fn switch_to_scheduler(&mut self) {
        if let Some(task_handle) = self.current_task.take() {
            let task_stack_pointer_reference = TASK_MANAGER.lock().borrow_mut().get_task_stack_pointer_ref(task_handle);
            self.current_task = Some(task_handle);
            let scheduler_stack_pointer = TASK_MANAGER.lock().borrow().get_task_stack_pointer(self.main_thread);
